...
- **`vb_laplace_vs_nuts_jit.json` fields** (all five required):
  - `per_stage_compile_events` — cold + warm event counts per kernel (LBFGS body, Hessian, NUTS step), sourced from JAX_LOG_COMPILES log parsing. **Note**: JAX_LOG_COMPILES does NOT emit per-event wall-clock timing; per-stage timing would require harness-splitting which this section explicitly rejects (full-pipeline-only). Field was originally `per_stage_compile_times` but was renamed to `per_stage_compile_events` when B4 verification found JAX_LOG_COMPILES emits event counts, not timings.
  - `per_call_wall_clock` — `{cold_s, warm_s, next_proc_s}` populated from `time.perf_counter()` wrappers around the full-pipeline `fit_vb_laplace_patrl` calls (first in-process cold, second in-process warm, third subprocess next-proc). Sibling to `per_stage_compile_events`; full-pipeline wall-clock is the timing signal when per-stage timing is not emit-able.
  - `per_p_scaling` — same stages at each P ∈ {5, 20} (or early-stopped subset — see P-sweep below); cross-references jit_scaling_sweep.csv
...
- **P-sweep cluster job structure**: SLURM array with `--dependency=afterok` chain. For each P ∈ {5, 20, 50}: job A cold (populates cache), job B next-proc warm (reads cache). Array job automated so the executor isn't hand-submitting 8 jobs sequentially. Each job capped at ≤30min walltime (phase SC 7). **P=2 dropped: too small to be informative per planner judgment; CONTEXT 'iterate by increasing' directive honored by ascending 5 → 20 → 50.**

; ModuleID = 'jit__unstack'
source_filename = "jit__unstack"
target datalayout = "e-i64:64-i128:128-v16:16-v32:32-n16:32:64"
target triple = "nvptx64-nvidia-cuda"

define void @wrapped_slice_2(ptr noalias align 16 dereferenceable(24) %arg0, ptr noalias align 128 dereferenceable(8) %arg1) {
entry:
  %0 = call i32 @llvm.nvvm.read.ptx.sreg.ctaid.x(), !range !6
  %1 = call i32 @llvm.nvvm.read.ptx.sreg.tid.x(), !range !7
  %2 = mul nuw nsw i32 %0, 2
  %linear_index = add nuw nsw i32 %2, %1
  %linear_index_in_range = icmp ult i32 %linear_index, 2
  call void @llvm.assume(i1 %linear_index_in_range)
  %linear_index0 = add nuw nsw i32 %linear_index, 0
  %3 = udiv i32 %linear_index0, 1
  %4 = urem i32 %3, 2
  %5 = udiv i32 %linear_index0, 2
  %6 = icmp ult i32 %linear_index, 2
  br i1 %6, label %wrapped_slice.2.in_bounds-true, label %wrapped_slice.2.in_bounds-after

wrapped_slice.2.in_bounds-after:                  ; preds = %wrapped_slice.2.in_bounds-true, %entry
  ret void

wrapped_slice.2.in_bounds-true:                   ; preds = %entry
  %7 = add i32 %5, 2
  %8 = add i32 %4, 0
  %9 = getelementptr inbounds [3 x [2 x i32]], ptr %arg0, i32 0, i32 %7, i32 %8
  %10 = load i32, ptr %9, align 4, !invariant.load !8
  %11 = getelementptr i32, ptr %arg1, i32 %linear_index
  %12 = getelementptr inbounds i32, ptr %11, i32 0
  store i32 %10, ptr %12, align 4
  br label %wrapped_slice.2.in_bounds-after
}

; Function Attrs: nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare noundef i32 @llvm.nvvm.read.ptx.sreg.ctaid.x() #0

; Function Attrs: nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare noundef i32 @llvm.nvvm.read.ptx.sreg.tid.x() #0

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(inaccessiblemem: write)
declare void @llvm.assume(i1 noundef) #1

define void @wrapped_slice_1(ptr noalias align 16 dereferenceable(24) %arg0, ptr noalias align 128 dereferenceable(8) %arg1) {
entry:
  %0 = call i32 @llvm.nvvm.read.ptx.sreg.ctaid.x(), !range !6
  %1 = call i32 @llvm.nvvm.read.ptx.sreg.tid.x(), !range !7
  %2 = mul nuw nsw i32 %0, 2
  %linear_index = add nuw nsw i32 %2, %1
  %linear_index_in_range = icmp ult i32 %linear_index, 2
  call void @llvm.assume(i1 %linear_index_in_range)
  %linear_index0 = add nuw nsw i32 %linear_index, 0
  %3 = udiv i32 %linear_index0, 1
  %4 = urem i32 %3, 2
  %5 = udiv i32 %linear_index0, 2
  %6 = icmp ult i32 %linear_index, 2
  br i1 %6, label %wrapped_slice.1.in_bounds-true, label %wrapped_slice.1.in_bounds-after

wrapped_slice.1.in_bounds-after:                  ; preds = %wrapped_slice.1.in_bounds-true, %entry
  ret void

wrapped_slice.1.in_bounds-true:                   ; preds = %entry
  %7 = add i32 %5, 1
  %8 = add i32 %4, 0
  %9 = getelementptr inbounds [3 x [2 x i32]], ptr %arg0, i32 0, i32 %7, i32 %8
  %10 = load i32, ptr %9, align 4, !invariant.load !8
  %11 = getelementptr i32, ptr %arg1, i32 %linear_index
  %12 = getelementptr inbounds i32, ptr %11, i32 0
  store i32 %10, ptr %12, align 4
  br label %wrapped_slice.1.in_bounds-after
}

define void @wrapped_slice(ptr noalias align 16 dereferenceable(24) %arg0, ptr noalias align 128 dereferenceable(8) %arg1) {
entry:
  %0 = call i32 @llvm.nvvm.read.ptx.sreg.ctaid.x(), !range !6
  %1 = call i32 @llvm.nvvm.read.ptx.sreg.tid.x(), !range !7
  %2 = mul nuw nsw i32 %0, 2
  %linear_index = add nuw nsw i32 %2, %1
  %linear_index_in_range = icmp ult i32 %linear_index, 2
  call void @llvm.assume(i1 %linear_index_in_range)
  %linear_index0 = add nuw nsw i32 %linear_index, 0
  %3 = udiv i32 %linear_index0, 1
  %4 = urem i32 %3, 2
  %5 = udiv i32 %linear_index0, 2
  %6 = icmp ult i32 %linear_index, 2
  br i1 %6, label %wrapped_slice.in_bounds-true, label %wrapped_slice.in_bounds-after

wrapped_slice.in_bounds-after:                    ; preds = %wrapped_slice.in_bounds-true, %entry
  ret void

wrapped_slice.in_bounds-true:                     ; preds = %entry
  %7 = add i32 %5, 0
  %8 = add i32 %4, 0
  %9 = getelementptr inbounds [3 x [2 x i32]], ptr %arg0, i32 0, i32 %7, i32 %8
  %10 = load i32, ptr %9, align 4, !invariant.load !8
  %11 = getelementptr i32, ptr %arg1, i32 %linear_index
  %12 = getelementptr inbounds i32, ptr %11, i32 0
  store i32 %10, ptr %12, align 4
  br label %wrapped_slice.in_bounds-after
}

attributes #0 = { nocallback nofree nosync nounwind speculatable willreturn memory(none) }
attributes #1 = { nocallback nofree nosync nounwind willreturn memory(inaccessiblemem: write) }

!nvvm.annotations = !{!0, !1, !2, !3, !4, !5}

!0 = !{ptr @wrapped_slice_2, !"kernel", i32 1}
!1 = !{ptr @wrapped_slice_2, !"reqntidx", i32 2}
!2 = !{ptr @wrapped_slice_1, !"kernel", i32 1}
!3 = !{ptr @wrapped_slice_1, !"reqntidx", i32 2}
!4 = !{ptr @wrapped_slice, !"kernel", i32 1}
!5 = !{ptr @wrapped_slice, !"reqntidx", i32 2}
!6 = !{i32 0, i32 1}
!7 = !{i32 0, i32 2}
!8 = !{}

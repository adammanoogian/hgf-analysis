...
**Current focus:** Phase 21 COMPLETE (7/7 plans) — benchmark bottleneck diagnosis. VERDICT.md published naming `cache_key` (per-node cache scope vs SLURM scheduler mismatch) as dominant bottleneck. Fix surface: `cluster/14_benchmark_gpu.slurm:99` (node-pinning via `#SBATCH --nodelist` + delete N=2 Phase-1 warm-up). Follow-up plan 14.1-07 (gap closure) unblocks 14.1-03 (BlackJAX benchmark resubmission).
...
[===========██████████��█████]   v1.1 code-complete (Phases 1-11); Phases 12-14 verified; Phase 16 complete; Phase 17 complete; Phase 18 complete (6/6); Phase 19 COMPLETE (5/5); Phase 14.1 gap closure in progress (1/6) — 14.1-07 added as Phase 21 verdict consumer; Phase 20 COMPLETE (8/8); Phase 21 COMPLETE (7/7)
...
| JAX persistent cache is per-node by design in SLURM script (JAX_COMPILATION_CACHE_DIR=.../.jax_cache_gpu/${SLURMD_NODENAME:-local}); cross-SLURM-job reuse requires --nodelist pinning | User-pasted cluster listing confirms 13 per-node cache subdirs totalling 1.8G; Phase 14 job 54899271 routed into m3a118/; subsequent jobs on different nodes get empty caches. Per-node scope is INTENTIONAL (avoids same-node concurrent-write contention) but defeats persistent-cache-across-jobs promise. Fix surface = cluster infra (node pinning / network-shared mount / single-subprocess restructuring), NOT production code. | 21-06 |
| HLO vmap scan body shape-nonpolymorphic across P (Jaccard=0.180 between P=5/P=20/P=50 pscan cold runs; scan-body sha256[:16] hashes all differ); N=2 warm-up cannot reuse N=50 kernels | Phase 14 14_benchmark_gpu.slurm Phase-1 warm-up is structurally orthogonal to Phase-2 production measurement — 15-byte text-IR delta drives XLA cache-key divergence; secondary independent fix recommendation for 21-07: delete the N=2 warm-up step | 21-06 |
| Phase 21 VERDICT: dominant bottleneck is `cache_key` — per-node cache scope vs SLURM scheduler mismatch (not closure-over-data, not graph size, not sampler kernel) | At P=50 next_proc_warm_s/cold_jit_s = 1.00x (229s/229s) at every tested P; HLO text-IR + XLA cache keys bit-identical across processes (falsifies closure-over-data); 13 per-node cache subdirs × 1.8G total + Phase 14 routed to m3a118/ = SLURM reassignment across afterok chains defeats persistent cache. See VERDICT.md. | 21 |
| Phase 21 fix surface: `cluster/14_benchmark_gpu.slurm:99` SBATCH directives (add `--nodelist=<pinned>`) + delete Phase-1 N=2 warm-up block — NOT production `src/` code | Two-line SBATCH patch + one-block deletion; zero risk of concurrent-write contention (option A preferred over option B's network-shared mount); closure-over-data refactor of hierarchical.py:1144 _one_step is NOT warranted (21-06 HLO identity evidence falsifies RESEARCH Risk #5) | 21 |
| Phase 21 follow-up plan: new `14.1-07` gap closure (node-pin + verify before 14.1-03 resubmission) | Right size: larger than quick-NNN (needs a P=50 verification SLURM submission + SUMMARY to capture next_proc/cold ratio); smaller than new integer phase (two-line SBATCH patch). 14.1-03 CANNOT proceed as-is — must wait for 14.1-07 to land node-pinning patch. | 21 |
...
Last session: 2026-04-19 (Phase 21 COMPLETE — VERDICT.md published)
Stopped at: Completed plan 21-07. VERDICT.md (~460 lines, 8 mandatory sections a-h) names `cache_key` as dominant bottleneck — per-node cache scope via JAX_COMPILATION_CACHE_DIR=.../.jax_cache_gpu/${SLURMD_NODENAME} combined with SLURM no-node-affinity across afterok-chained jobs. PR-level fix: cluster/14_benchmark_gpu.slurm:99 (add #SBATCH --nodelist=<pinned> OR drop SLURMD_NODENAME suffix) + delete Phase-1 N=2 warm-up. Follow-up plan: 14.1-07. Success criterion: next_proc_warm_s/cold_jit_s >= 3.0x at P=50 (baseline 1.00x). vb_laplace_vs_nuts_jit.json bottleneck_verdict.bottleneck_layer = 'cache_key'; hlo_op_counts filled (16 keys). ROADMAP + CONTEXT synced (B1/B2/B3/B4/M1 checker blockers resolved).
Resume file: None
Next action: 14.1-07 gap-closure plan (node-pin + verify) OR direct resubmission of 14.1-03 once node-pinning patch lands on origin/main. 14.1-03 CANNOT proceed as-is — must wait for 14.1-07. Still pending: Cluster 160-agent PRL-V1/V2 numeric runs (`sbatch cluster/patrl_smoke.slurm` with Laplace default) — deferred until Phase 14.1 benchmark closes so production partition/walltime is known.

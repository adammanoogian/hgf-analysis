...
**Goal**: Identify the **root cause** of the Phase 14 BlackJAX GPU benchmark bottleneck observed in job 54899271 (cold JIT 2197s, warm JIT 1965s → only 1.1x cache speedup; timed out at 8h on N=50 Phase 2 before writing `benchmark_batched.json`). Produce the diagnostic evidence needed to pick a principled fix rather than resolving via walltime bump or participant-count reduction. This phase is **purely diagnostic** — it does NOT re-execute the Phase 14 production benchmark. 14.1-03 remains a separate human-in-the-loop cluster resubmission after this phase names the fix. Deliverables: a compute-graph scope memo that quantifies this model's size vs standard Bayesian/ML graphs + a FLOPs/draw estimate; a VB-Laplace-vs-NUTS JIT comparison that isolates whether the bottleneck is the logp graph or the sampler; a JAX persistent-cache forensics audit that determines whether the warm run truly hit the cache or cold-retraced; an iterative P-sweep that maps JIT time vs cohort size so the Phase 14 Phase-1 smoke measures cold→warm at the production shape, not at N=2.

**Depends on**: Phase 14 (14.1-01 complete; SLURM SAMPLER env-var + git auto-push); consumes the `_build_patrl_log_posterior` and `build_logp_fn_batched` pure-JAX logp surfaces in place today. Sister to Phase 14.1-03 (which still needs a successful bench run to write `results/power/benchmark_batched.json`).

**Requirements**: BENCH-DIAG-01 (graph scope memo), BENCH-DIAG-02 (SVI probe), BENCH-DIAG-03 (cache key forensics), BENCH-DIAG-04 (P-sweep), BENCH-DIAG-05 (root-cause verdict + fix recommendation).

**Success Criteria** (what must be TRUE):
  1. **Graph scope memo** at `.planning/phases/21-benchmark-bottleneck-diagnosis/benchmark-scope-memo.md` contains (a) a markdown comparison table of traced-HLO size + FLOPs/draw for the batched hierarchical PAT-RL logp (P × 192 trials × pyhgf Network message passing: `value_prediction_error`, `effective_precision`, `volatility_prediction_error`, per-trial continuous-node posterior updates, volatility-parent couplings) and the pick_best_cue variant (P × 420 trials × 3-cue partial feedback), against state-space model baselines (NumPyro HMMs, Kalman-filter scan-based MCMC fits, LGSSM examples) — closest structural match to HGF's per-trial scan-in-logp, per CONTEXT.md §Comparison baselines; (b) a breakdown of what message passing is actually required per leapfrog step: per-trial HGF state updates, per-participant vmapped scan, per-draw NUTS leapfrog (≈ 2× logp grad evaluations × integration steps), per-chain pmap/vmap; (c) a first-principles FLOPs/draw estimate with the assumptions listed explicitly (P=50, T=192, trial-scan ops count, NUTS step depth)
  2. **VB-Laplace vs NUTS JIT comparison** at `.planning/phases/21-benchmark-bottleneck-diagnosis/vb_laplace_vs_nuts_jit.json` records cold and warm JIT compile events and per-call wall-clock times for VB-Laplace (reusing `fit_vb_laplace_patrl` from Phase 19 — the already-validated TAPAS-parity quasi-Newton MAP + `jax.hessian` Laplace covariance path; zero new deps) and `blackjax.nuts` (window adaptation, ≈10 draws) on the **identical** `_build_patrl_log_posterior` closure at matched shapes. Diagnostic rule: if VB-Laplace compiles slow (>5 min cold on GPU), bottleneck is the logp graph (scan body, vmap shape, closure over pyhgf Network); if VB-Laplace compiles fast and NUTS still slow, bottleneck is the NUTS step function. Verdict recorded in the JSON under `"bottleneck_verdict.bottleneck_layer"` with values in `{"logp_graph", "lbfgs_loop", "hessian_graph", "nuts_kernel", "cache_key", "both", "undetermined"}`. Per CONTEXT.md §VB-Laplace probe design, reusing the Phase 19 validated Laplace path replaces the originally-proposed SVI probe to avoid introducing numpyro.SVI as a new diagnostic dep.
  3. **Cache key forensics** at `.planning/phases/21-benchmark-bottleneck-diagnosis/cache_forensics.md` answers: (a) does `/scratch/fc37/aman0087/.jax_cache_gpu/m3a118` actually contain populated `.jax_cache` entries after a Phase-1 cold run (listing + byte counts)? (b) is HLO hashing keying on something that changes run-to-run — trace-time constants, closure values, host PID, randomized scheduling? (enumerate via `JAX_LOG_COMPILES=1` + `XLA_FLAGS=--xla_dump_hlo_as_text --xla_dump_to=/tmp/hlo_dump` on two back-to-back runs and diff the HLO hashes); (c) is Phase-1's N=2 kernel ever reused by Phase-2's N=50 call, or is that structurally a different HLO and thus cache-independent by design (answer with HLO hash evidence)? Verdict on why warm was only ~10% faster than cold
  4. **P-sweep JIT scaling** at `.planning/phases/21-benchmark-bottleneck-diagnosis/jit_scaling_sweep.csv` with one row per P ∈ {5, 20, 50} (or a subset if the L40S hits VRAM limits sooner), columns `P, cold_jit_s, same_proc_warm_s, next_proc_warm_s, draws_per_s, vram_peak_mb, status` where `status ∈ {completed, early_stopped, infeasible}`. **P=2 dropped: too small to be informative per planner judgment; CONTEXT 'iterate by increasing' directive honored by ascending 5 → 20 → 50. Status column replaced wall-clock-duplicate per planner judgment; early-stop visibility more valuable than redundant timing.** Each P runs as a **separate short-walltime SLURM job** (≤30 min) using the existing `14_benchmark_gpu.slurm` infrastructure with a new `--p-scan P` CLI that forces a single-P run and writes one CSV row. Plot `log(jit_s)` vs `log(P)` — linear slope ≈ 1 is acceptable trace-time scaling; slope ≫ 1 or super-exponential indicates a graph blow-up to fix. `next_proc_warm_s` (second SLURM job same shape, same cache dir) is the authoritative persistent-cache measurement; `same_proc_warm_s` (second call within one Python process) is the in-process JIT trace cache measurement. If `next_proc_warm_s > same_proc_warm_s × 2`, the persistent cache is not landing
  5. **Root-cause verdict + fix recommendation** at `.planning/phases/21-benchmark-bottleneck-diagnosis/VERDICT.md` names the single dominant bottleneck among {NUTS sampler step, HGF scan graph, JIT cache key, closure-over-data vs traced-args, shape polymorphism, other} with cited evidence from SCs 1-4, and specifies a concrete fix unblocking 14.1-03: e.g. "rewrite Phase-1 smoke to measure cold→warm at production shape" / "port PAT-RL closure path to traced-args sample loop (STATE #107 pattern)" / "switch to `blackjax.inner_kernel_tuning` to drop leapfrog integration-steps unrolling" / "break the big HGF scan into smaller scans via `lax.scan` nesting". Recommendation is actionable at the PR level — it names the file, the function, and the patch shape
...
Plans:
- [x] 21-01-PLAN.md — graph scope memo (HLO op counts + scan-body depth + FLOPs/draw + state-space comparison)
- [x] 21-02-PLAN.md — re-instrument _run_benchmark (JAX_LOG_COMPILES + cache-delta tracking + warmup_params) + add --p-scan and --vb-laplace-probe CLI flags
- [x] 21-03-PLAN.md — VB-Laplace GPU probe at P in {5, 20} (cluster/21_diagnose_laplace.slurm)
- [x] 21-04-PLAN.md — NUTS (PAT-RL) GPU probe at P in {5, 20} matched to Laplace shapes (cluster/21_diagnose_nuts.slurm)
- [x] 21-05-PLAN.md — P-sweep chain at P in {5, 20, 50} with afterok cold→warm dependencies (cluster/21_diagnose_pscan.slurm + 21_submit_pscan_chain.sh)
- [x] 21-06-PLAN.md — cache forensics: HLO hash matrix + cache-dir listing + N=2-vs-N=50 verdict + 1.1x speedup narrative
- [x] 21-07-PLAN.md — VERDICT.md synthesis: name single dominant bottleneck + PR-level fix recommendation + 14.1-03 unblock signal
...
- STATE.md decision #105: "JIT gate thresholds preserved at NumPyro levels with annotation — Thresholds conservative for BlackJAX; will tighten after cluster benchmarking" — this phase produces the data that unblocks that tightening
- STATE.md decision #106/107: the `_build_sample_loop` factory pattern that keys XLA persistent cache on HLO hash by passing data as traced JIT args — pick_best_cue uses this path; PAT-RL uses the closure-based logdensity_fn path (decision #119) for parallel-stack reasons. The cache-forensics SC must determine whether the closure-path HLO hash is cache-stable run-to-run
- pyhgf 0.2.8 `net.scan_fn`: the per-trial update function that dominates the inner loop. Message passing confirmed via decision #126 (`value_prediction_error`, `effective_precision`, `volatility_prediction_error` all present)
- `src/prl_hgf/fitting/hierarchical.py::_build_log_posterior` (pick_best_cue) and `src/prl_hgf/fitting/hierarchical_patrl.py::_build_patrl_log_posterior` (PAT-RL) — the two logp surfaces to instrument
...
- `JAX_LOG_COMPILES=1` + `XLA_FLAGS=--xla_dump_hlo_as_text` — the cache forensics instrumentation (SC3)

**Scope notes**:
- This phase is **strictly diagnostic**. It produces evidence and a verdict. It does NOT implement the fix. The fix lives in a separate follow-up plan (either a 14.1-0X gap-closure, or a new integer phase depending on scope) that references this phase's VERDICT.md
- **Do not resolve by bumping walltime or shrinking the production cohort** — that is the banned shortcut. Shrinking participant count is acceptable for SC2/SC4 *smoke timing* but not as the production answer
- **Phase is sister to 14.1-03**, not a replacement. 14.1-03 still requires a successful production-sampler benchmark write of `results/power/benchmark_batched.json` with finite `per_iteration_s` and `gpu_hours_per_chunk` — this phase makes that run cheap and principled rather than speculative
- Cluster vs local split: SCs 2-4 require real GPU (A100 or L40S) for representative JIT timings. SC1 (graph scope memo) is local-only desk research. SC5 (verdict) is synthesis, local-only. Plan during `/gsd:plan-phase 21` should wave these into (a) local graph-scope memo first, (b) cluster VB-Laplace + NUTS probes + cache forensics next, (c) P-sweep CSV population last, (d) VERDICT.md synthesis at close
- **Explicitly named anti-patterns the planner must avoid**: resubmitting `cluster/14_benchmark_gpu.slurm` with `--time=24:00:00`; reducing `max_n` in the grid as a "fix" (only acceptable for the diagnostic smoke timings); patching `apply_decision_gate` thresholds before the verdict is in

**Architectural directives (USER-SPECIFIED)**:
- **Iterate by increasing parameters, not by decreasing them.** The P-sweep goes 2 → 5 → 20 → 50. Reducing participants is a smoke-test diagnostic only; the production decision is about whether P=50 compiles at all, in reasonable time, and with cache re-hits across SLURM jobs
- **Isolate before fixing.** VB-Laplace probe must land before any production-sampler code change. If VB-Laplace compiles fast, a sampler-level fix (BlackJAX step-function restructure, leapfrog unrolling) is warranted. If VB-Laplace compiles slow, a graph-level fix (HGF scan body, vmap shape, closure plumbing) is warranted. Don't guess — measure
- **Cache forensics beats cache-hypothesis.** Warm-3min-faster-than-cold is not a cache hit. Get the HLO hashes, list the cache dir bytes, confirm the `.jax_cache` lookup actually fired — then either celebrate or fix the cache key

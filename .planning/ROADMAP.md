...
**Success Criteria** (what must be TRUE):
  1. **Graph scope memo** at `.planning/phases/21-benchmark-bottleneck-diagnosis/benchmark-scope-memo.md` contains (a) a markdown comparison table of traced-HLO size + FLOPs/draw for the batched hierarchical PAT-RL logp (P × 192 trials × pyhgf Network message passing: `value_prediction_error`, `effective_precision`, `volatility_prediction_error`, per-trial continuous-node posterior updates, volatility-parent couplings) and the pick_best_cue variant (P × 420 trials × 3-cue partial feedback), against at minimum: Stan/NumPyro 8-schools, Radon-hierarchical, a BNN with ~1M params, a transformer forward pass (ballpark), and a standard CNN forward pass (ballpark); (b) a breakdown of what message passing is actually required per leapfrog step: per-trial HGF state updates, per-participant vmapped scan, per-draw NUTS leapfrog (≈ 2× logp grad evaluations × integration steps), per-chain pmap/vmap; (c) a first-principles FLOPs/draw estimate with the assumptions listed explicitly (P=50, T=192, trial-scan ops count, NUTS step depth)
  2. **SVI vs NUTS JIT comparison** at `.planning/phases/21-benchmark-bottleneck-diagnosis/svi_vs_nuts_jit.json` records cold and warm JIT compile times for `numpyro.infer.SVI` (Normal guide, ≈200 iterations) and `blackjax.nuts` (window adaptation, ≈10 draws) on the **identical** `_build_patrl_log_posterior` closure at matched shapes. Diagnostic rule: if SVI compiles slow (>5 min cold), bottleneck is the logp graph (scan body, vmap shape, closure over pyhgf Network); if SVI compiles fast (<60s) and NUTS still slow, the bottleneck is the NUTS step function (leapfrog integrator, integration-steps unrolling, step-size adaptation). Verdict recorded in the JSON under `"bottleneck_layer"` with values in `{"logp_graph", "nuts_sampler", "both", "undetermined"}`
...
- Cluster vs local split: SCs 2-4 require real GPU (A100 or L40S) for representative JIT timings. SC1 (graph scope memo) is local-only desk research. SC5 (verdict) is synthesis, local-only. Plan during `/gsd:plan-phase 21` should wave these into (a) local graph-scope memo first, (b) cluster SVI probe + cache forensics next, (c) P-sweep CSV population last, (d) VERDICT.md synthesis at close
...
- **Isolate before fixing.** SVI probe must land before any production-sampler code change. If SVI compiles fast, a sampler-level fix (BlackJAX step-function restructure, leapfrog unrolling) is warranted. If SVI compiles slow, a graph-level fix (HGF scan body, vmap shape, closure plumbing) is warranted. Don't guess — measure
